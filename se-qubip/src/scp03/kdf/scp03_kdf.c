...
static void scp03_kdf_internal(int key_bits,
                             unsigned char* key,
                               ...
                               unsigned char* session_rmac)
{
    ...
    scp03_kdf_internal(key_bits, static_enc_key, SCP03_CONST_ENC, key_bits, host_challenge, card_challenge, session_enc);
                     
    // Derive S-MAC (Uses Static K-MAC)
    scp03_kdf_internal(key_bits, static_mac_key, SCP03_CONST_MAC, key_bits, host_challenge, card_challenge, session_mac);
                     
    // Derive S-RMAC (Uses Static K-MAC)
    scp03_kdf_internal(key_bits, static_mac_key, SCP03_CONST_RMAC, key_bits, host_challenge, card_challenge, session_rmac);
}
