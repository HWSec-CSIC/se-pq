...
{
    for(int i=0; i<8; i++) bytes[i] = (val >> ((7-i)*8)) & 0xFF;
}
...
{
    uint64_t val = 0;
    for(int i=0; i<8; i++) val |= ((uint64_t)bytes[i] << ((7-i)*8));
    return val;
}
...
static void i2c_wait_data(I2C_FD interface) 
{
    ...
    {
        uint8_t status = read_I2C_status(interface);
        if (status & STATUS_DATA_AVAIL) break;
    }
}
                  ...
                  unsigned char* ext_auth_out)
{
    ...
    scp03_derive_session_keys(session->key_bits, 
                              session->static_enc, session->static_mac, 
                              session->host_chal, session->card_chal, 
    ...
    scp03_calc_cryptograms(session->key_bits, session->s_mac, 
                           session->host_chal, session->card_chal,
                           calc_card_crypt, calc_host_crypt);
    ...
    // Block 1: Chain (00...)
    memcpy(&mac_input[ptr], session->mac_chain, 16); ptr += 16;
    ...
    memcpy(&mac_input[ptr], header, 5); ptr += 5;
    memcpy(&mac_input[ptr], calc_host_crypt, 16); ptr += 16;
    
    // Pad to 16 bytes
    mac_input[ptr++] = 0x80;
    while (ptr % 16 != 0) mac_input[ptr++] = 0x00;
    
    // Calculate C-MAC
    unsigned int mac_len;
    unsigned char full_mac[16];

    if      (session->key_bits == 128)  AES_128_CMAC(session->s_mac, full_mac, &mac_len, mac_input, ptr);
    else if (session->key_bits == 192)  AES_192_CMAC(session->s_mac, full_mac, &mac_len, mac_input, ptr);
    else if (session->key_bits == 256)  AES_256_CMAC(session->s_mac, full_mac, &mac_len, mac_input, ptr);
 ...
 */
static int scp03_send_data(scp03_session_t* session,
                    unsigned char* header,
                    unsigned char* payload,
                    int payload_len,
                    unsigned char* apdu_out) 
{
    unsigned char iv[16];
    ...
    // A. Encrypt Payload
    // 1. Calculate IV
    if      (session->key_bits == 128)  AES_128_CBC_ENCRYPT(session->s_enc, zero_iv, iv, &out_len, session->counter, 16);
    else if (session->key_bits == 192)  AES_192_CBC_ENCRYPT(session->s_enc, zero_iv, iv, &out_len, session->counter, 16);
    else if (session->key_bits == 256)  AES_256_CBC_ENCRYPT(session->s_enc, zero_iv, iv, &out_len, session->counter, 16);
    ...
    while (pad_len % 16 != 0) padded_pt[pad_len++] = 0x00;
    
    // 3. Encrypt Data
    unsigned char ciphertext[256];
    if      (session->key_bits == 128)  AES_128_CBC_ENCRYPT(session->s_enc, iv, ciphertext, &out_len, padded_pt, pad_len);
    else if (session->key_bits == 192)  AES_192_CBC_ENCRYPT(session->s_enc, iv, ciphertext, &out_len, padded_pt, pad_len);
    else if (session->key_bits == 256)  AES_256_CBC_ENCRYPT(session->s_enc, iv, ciphertext, &out_len, padded_pt, pad_len);
    ...
    memcpy(&mac_input[ptr], mod_header, 5); ptr += 5;
    
    // 3. Ciphertext
    memcpy(&mac_input[ptr], ciphertext, cipher_len); ptr += cipher_len;
    
    // 4. CMAC
    unsigned char full_mac[16];
    unsigned int mac_len_out;
    if      (session->key_bits == 128)  AES_128_CMAC(session->s_mac, full_mac, &mac_len_out, mac_input, ptr);
    else if (session->key_bits == 192)  AES_192_CMAC(session->s_mac, full_mac, &mac_len_out, mac_input, ptr);
    else if (session->key_bits == 256)  AES_256_CMAC(session->s_mac, full_mac, &mac_len_out, mac_input, ptr);
                    ...
                    unsigned char* plaintext_out)
{
    ...
    if      (session->key_bits == 128)  AES_128_CBC_ENCRYPT(session->s_enc, zero_iv, iv, &out_len, iv_input, 16);
    else if (session->key_bits == 192)  AES_192_CBC_ENCRYPT(session->s_enc, zero_iv, iv, &out_len, iv_input, 16);
    else if (session->key_bits == 256)  AES_256_CBC_ENCRYPT(session->s_enc, zero_iv, iv, &out_len, iv_input, 16);
    inc_counter(session->counter);
    ...
    // 2. Ciphertext
    memcpy(&mac_input[ptr], ciphertext, 16); ptr += 16;
    ...
    unsigned char full_mac[16];
    unsigned int mac_len;
    if      (session->key_bits == 128)  AES_128_CMAC(session->s_rmac, full_mac, &mac_len, mac_input, ptr);
    else if (session->key_bits == 192)  AES_192_CMAC(session->s_rmac, full_mac, &mac_len, mac_input, ptr);
    else if (session->key_bits == 256)  AES_256_CMAC(session->s_rmac, full_mac, &mac_len, mac_input, ptr);
    ...
    unsigned char padded_pt[256];
    unsigned int pt_len;
    if      (session->key_bits == 128)  AES_128_CBC_DECRYPT(session->s_enc, iv, ciphertext, 16, padded_pt, &pt_len);
    else if (session->key_bits == 192)  AES_192_CBC_DECRYPT(session->s_enc, iv, ciphertext, 16, padded_pt, &pt_len);
    else if (session->key_bits == 256)  AES_256_CBC_DECRYPT(session->s_enc, iv, ciphertext, 16, padded_pt, &pt_len);

    // Remove Padding
    unsigned int real_len = 16;
    while (real_len > 0 && padded_pt[real_len-1] == 0x00) real_len--;
    if (real_len > 0 && padded_pt[real_len-1] == 0x80) real_len--;
