...
void save_data_flash(unsigned int addr, unsigned int data_len, unsigned char* data_in, INTF interface)
{
	...
	for (uint32_t i = 0; i < data_pages; i++)
	{
		...
		{
			write_INTF(interface, data_in + (j + packages_write) * AXI_BYTES, PICORV32_DATA_IN, AXI_BYTES);
		}
	...
	{
		write_INTF(interface, data_in + (i + packages_write) * AXI_BYTES, PICORV32_DATA_IN, AXI_BYTES);
	}
...
void recover_data_flash(unsigned int addr, unsigned int data_len, unsigned char* data_out, INTF interface)
{
	...
	for (uint32_t i = 0; i < data_blocks; i++)
	{
		...
		{
			read_INTF(interface, data_out + (j + packages_read) * AXI_BYTES, PICORV32_DATA_OUT, AXI_BYTES);
		}
	...
	for (uint32_t i = 0; i < data_rem; i++)
	{
		read_INTF(interface, data_out + (i + packages_read) * AXI_BYTES, PICORV32_DATA_OUT, AXI_BYTES);
	}
