...
void x25519_ss_gen_hw(unsigned char **shared_secret, unsigned int *shared_secret_len, unsigned char *pub_key, unsigned int pub_len, unsigned char *pri_key, unsigned int pri_len, MMIO_WINDOW ms2xl)
{
    ...
    */
    ...
    //-- Write Scalar and Input Point
    x25519_write(X25519_SCALAR, X25519_BYTES / AXI_BYTES, pri_key, X25519_RST_ON, ms2xl);
    x25519_write(X25519_POINT_IN, X25519_BYTES / AXI_BYTES, pub_key, X25519_RST_ON, ms2xl);
    ...
    x25519_read(X25519_POINT_OUT, X25519_BYTES / AXI_BYTES, *shared_secret, ms2xl);
