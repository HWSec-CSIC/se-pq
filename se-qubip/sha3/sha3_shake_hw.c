...
{
	sha3_shake_hw(in, out, length, 256, 1, 1088, 256, ms2xl, 0);
}

void sha3_512_hw_func(unsigned char* in, unsigned char* out, unsigned int length, PYNQ_MMIO_WINDOW ms2xl)
{
	sha3_shake_hw(in, out, length, 512, 2, 576, 512, ms2xl, 0);
}
...
{
	sha3_shake_hw(in, out, length, length_out, 3, 1344, 128, ms2xl, 0);
}
...
{
	sha3_shake_hw(in, out, length, length_out, 4, 1088, 256, ms2xl, 0);
}
...
void sha3_shake_ms2xl(unsigned long long int* a, unsigned long long int* b, PYNQ_MMIO_WINDOW ms2xl, unsigned int pos_pad, int pad, int shake, int VERSION, int SIZE_SHA3, int SIZE_BLOCK, int DBG) {
	...
	if (shake != 1) {
		if (pad) {
			...
			op = op_version | LOAD_LENGTH; // LOAD
	...
	if (pad) {
		...
		op = op_version | LOAD_LENGTH; // ENABLE_SHAKE
		PYNQ_writeMMIO(&ms2xl, &op, CONTROL, sizeof(unsigned long long int));
