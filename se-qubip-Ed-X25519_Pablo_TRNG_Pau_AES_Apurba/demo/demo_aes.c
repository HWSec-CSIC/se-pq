...
void demo_aes_hw(unsigned int bits, unsigned int verb, MMIO_WINDOW ms2xl) {
    ...
    if (bits == 128) {
        ...
        // --- CBC --- //
        aes_128_cbc_encrypt_hw(key_128, iv_128, ciphertext_128, &ciphertext_128_len, msg, strlen(msg), ms2xl);
        aes_128_cbc_decrypt_hw(key_128, iv_128, ciphertext_128, ciphertext_128_len, recovered_msg_128, &recovered_msg_128_len, ms2xl);
	...
	unsigned int plaintext_len = strlen(msg); //strlen((char *)msg);
	
	unsigned int recovered_msg_256_len = ((plaintext_len + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
	
	unsigned char *recovered_msg_256 = (unsigned char *)calloc(recovered_msg_256_len, sizeof(unsigned char));
	...
	// printf("Plaintext length: %u bytes\n", plaintext_len);
        ...
        unsigned int ciphertext_len = ((plaintext_len + BLOCK_SIZE - 1) / BLOCK_SIZE) * BLOCK_SIZE;
        
        //printf("Ciphertext length: %u bytes\n", ciphertext_len);
        
    	//unsigned char *ciphertext = (unsigned char *)malloc(ciphertext_len);
    	 // Allocate memory for ciphertext using calloc to initialize it to zero
	...
	if (!ciphertext) {
    	...
    	aes_256_ecb_encrypt_hw(key_256, ciphertext, &ciphertext_len, msg,plaintext_len,ms2xl);
	...
	*/
	...
	aes_256_ecb_decrypt_hw(key_256, ciphertext, ciphertext_len, recovered_msg_256, &recovered_msg_256_len, ms2xl);



    	// Print the decrypted plaintext in hexadecimal format
    	/*printf("Decrypted plaintext (hex): ");
    		for (unsigned int i = 0; i < recovered_msg_256_len; i++) {
        		printf("%02x ", recovered_msg_256[i]);
    		}*/
    	printf("\n"); 
    	
	//printf("\n Recovered Message:: %s", recovered_msg_256);
	...
	if (verb >= 1) printf("\n recover msg: %s", recovered_msg_256);
        ...
        // --- CBC --- //
        aes_256_cbc_encrypt_hw(key_256, iv_256, ciphertext, &ciphertext_len, msg, plaintext_len, ms2xl);
        aes_256_cbc_decrypt_hw(key_256, iv_256, ciphertext, ciphertext_len, recovered_msg_256, &recovered_msg_256_len, ms2xl);
